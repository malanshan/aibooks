...
from CNN_Demo import data_process
from CNN_Demo.text_cnn import TextCNN
...
def pre_process():
    """
    从data目录读取数据，按照随机方式将数据分成训练集和测试集。
    :return: 训练集x，训练集y，词汇表，测试集x，测试集y。
    """
    print("loading data...")
    ...
    # 将数据集按照比例dev_sample_percentage分割成训练数据集和测试数据集
...
if __name__ == '__main__':
    # 入口函数。当run带参数时，例如tf.app.run(fn1)，会执行fn1函数；run不带参数则执行main函数。
    tf.app.run()
